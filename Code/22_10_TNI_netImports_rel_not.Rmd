...
direction <- googlesheets4::read_sheet(link, sheet = 'Eora_match_SDG', range = 'D:I', col_types = 'cccccc') %>%
  dplyr::select(Filename, SDG_class, starts_with('direction')) %>%
...
save(direction, file = f)


direction_clean <- direction %>%
  dplyr::filter(!is.na(direction)) %>%
  dplyr::select(ind, SDG_class, direction) %>%
  as.data.frame()
...
for (i in 1:nrow(direction_clean)) {
  # print(i)
  ind <- direction_clean[i, 'ind']       %>% unlist() %>% paste(sep = '', collapse = ''); 
  drt <- direction_clean[i, 'direction'] %>% unlist() %>% paste(sep = '', collapse = '') %>% as.numeric()
  print(ind)
  print(drt)
  ...
  net_in      <- func_net_import_direction(df = df, direction_i = drt, frac = 0.025)
...
fname <- paste0(dir.flowScenario, 'net_imports_fromAll_', postfix, '_', n_ft_ind, '.RData'); fname
...
### compare
load("./Data/data_02_intermediate/dt02_flows/net_imports_fromAll_20220812con_45.RData")
net_imp_con <- net_imports_fromAll
load("./Data/data_02_intermediate/dt02_flows/net_imports_fromAll_20220812pro_45.RData")
...
ggplot() + geom_point(aes(x = net_imp_con_i$net_in, y = net_imp_pro_i$net_in)) + theme_bw() 
...
unique(net_imports_fromAll$ind)

test <- net_imports_fromAll %>%
...
map1 <- spillover_index_shp %>%
  ...
  scale_fill_distiller(palette = "Reds", direction = 1, na.value = 'black', name = 'Impact\nIndex') +
...
fname <- paste0(dir.fig, 'spillover/', fignm);
fname
...
```{r - fig2a - index rank county-color-2-group}
p <- spillover_index_top_bottom %>%
  dplyr::mutate(rank = case_when(
    index > 50 ~ "top 10", 
    TRUE ~ 'bottom 10')) %>%
  dplyr::mutate(group = ifelse(group %in% c("High income", "Upper middle income"), "High income", "Low income")) %>%
  ggplot(aes(x = reorder(name, index))) +
  geom_col(aes(y = index, fill = (rank)), show.legend = T) + 
  geom_point(aes(y = -2, color = group), show.legend = T, size = 3) + # size = 1.2, 
  # scale_fill_manual(values=c('#fee6ce', '#f16913'))+
...
fig <- ggarrange(p, labels = "a", font.label = list(size = 12), vjust = 1, hjust = 0); fig
fname <- paste0(dir.fig, 'spillover/', 'fig2a_spillover_top_bot_color_2grp_', postfix, '_', today,'.jpg'); fname
ggsave(filename = fname, plot = fig, width = width_2col/3*2, height = width_2col/2, units = 'cm', dpi = 600)
```
...
```{r - fig3ex - Index rank with MC categories}
...
fname <- paste0(dir.fig, 'spillover/', 'spillover_index_top_bottom_ByType_', postfix, '_', today, '.jpg'); fname
ggsave(filename = fname, plot = last_plot(), width = width_2col/3*2, height = width_2col/2, units = 'cm', dpi = 600)
```



```{r - score of SE, eval=FALSE, include=FALSE}
...
ft  <- googlesheets4::read_sheet(link, sheet = 1) %>%
  as.data.frame() %>%
  dplyr::filter(Used == '1', 
                !is.na(Filename),
                !grepl('\\?', Filename)) %>%
...
map_each_p
fname <- paste0(dir.fig, 'spillover/', 'map_spillover_norm_each_', yr, '_long_', today, '.jpg'); #fname
ggsave(filename = fname, plot = map_each_p, width = 8, height = 4.5, units = 'in', dpi = 300)
```
