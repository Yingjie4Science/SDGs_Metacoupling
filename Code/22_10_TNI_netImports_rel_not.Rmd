...
direction <- googlesheets4::read_sheet(link, sheet = 'Eora_match_SDG', range = 'C:J', col_types = 'cccccccc') %>%
...
direction_clean <- direction %>%
  ...
  # dplyr::filter(Used == 1) %>%
  dplyr::select(FT_Indicators, ind, Used, SDG_class, direction) %>%
  as.data.frame()
# str(direction_clean)
...
f <- paste0('./Data/data_02_intermediate/dt02_flows/', 'direction.xlsx')
...
n_p <- length(unique(map_each_ind$indicator))
n_row <- ceiling(n_p/6 + 0.01); n_row 
...
map_each_p <- map_each %>%
  ...
  facet_wrap(~ShortName, ncol = 6, nrow = n_row) +
  # ggtitle(yr)+
...
ggsave(filename = fname, plot = map_each_p, width = 8, height = 4.5/6*n_row, units = 'in', dpi = 300)
```
