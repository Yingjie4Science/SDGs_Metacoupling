...
# V(net)$size <- log(V(net)$size_node/10^3)
...
# plot the network:
plot(net3, vertex.cex=(net3 %v% "size_node")/7, vertex.col= net3 %v% "col")
...
l <- plot(net3, vertex.cex=(net3 %v% "size_node")/7, vertex.col="col")
plot(net3, vertex.cex=(net3 %v% "size_node")/7, vertex.col="col", coord=l)


# The network package also offers the option to edit a plot interactively,
# by setting the parameter interactive=T

plot(net3, vertex.cex=(net3 %v% "size_node")/7, vertex.col="col", interactive=T)
...
# The edge attribute 'type' and node attribute 'size_node' are 
...
ggraph(net, layout = "centrality", cent = graph.strength(net))+
  ...
  geom_node_point(aes(fill = group, size = size_node), shape = 21) +
...
plot( x=0:max(igraph::degree(net)), y=1-deg.dist, pch=19, cex=1.4, col="orange", 
...
vis.links$width <- log(links$weight) # line width
...
plot(net3.dyn, vertex.cex=(net3 %v% "size_node")/7, vertex.col="col")
...
render.d3movie(net3.dyn, usearrows = F, 
               ...
               edge.lwd = (net3.dyn %e% "weight")/3000, 
               edge.col = '#55555599',
