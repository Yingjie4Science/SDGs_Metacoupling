...
```{r !! Key para}
## mainly to deal with two EORA energy data issue, as the ft data are average 100 times larger than domestic consumption value
coef_correction_curb_10e_issue          <- 10^2 
coef_correction_curb_10e_issue_energy   <- 10^3
coef_correction_curb_10e_issue_gdp      <- 10^3
coef_correction_curb_10e_issue_material <- 10^3
coef_correction_curb_10e_issue_N        <- 10^3
coef_correction_curb_10e_issue_P        <- 10^2
```



```{r --- check - SDG change plot function}

func_plot_sdg_ScoreChange_byGroup <- function(df_score, bar_or_violin = 'bar') {
  ...
  ### Create a dodged violin plot with jittered points
  p2 <- ggplot(df_shp, aes(x = as.factor(goal), y = value_chg, fill = group)) +
    # Dodged violin plots
    geom_violin(position = position_dodge(width = 0.8), alpha = 0.3, trim = FALSE) +
    # Boxplots for summary
    geom_boxplot(position = position_dodge(width = 0.8), width = 0.2, alpha = 0.2, outlier.shape = NA) +
    # Jittered points
  ...
  if (bar_or_violin == 'bar') {
    return(p1)
  } else if (bar_or_violin == 'violin') {
    return(p2)
  } else if (bar_or_violin == 'both') {
    return(p)
  } else {
...
```{r --- check the dif between net imports vs domestic value}

func_plot_netImports_vs_rel <- function(df, x = 'value', y = 'fromAll') {
  ...
  rel_not_merge_check <- df %>%
    dplyr::mutate(
      x_abs = ifelse(!is.na(x), abs(x), NA),
      y_abs = ifelse(!is.na(y), abs(y), NA),
      dif_10e = y_abs/x_abs)

  selected_data <- rbind(
    rel_not_merge_check %>% slice_max(prop = 0.00, order_by = x), 
    rel_not_merge_check %>% slice_min(prop = 0.00, order_by = y),
    rel_not_merge_check %>% slice_max(prop = 0.05, order_by = dif_10e)
    )
  
  p <- df %>%
    # ggplot(data = ., aes(x = value_rel, y = value_not))+
    ggplot(data = ., aes(x = x, y = y, color = iso3))+
...
##' as suggested by Mengyu, we can use the diagonal data - domestic consumption as a reference
##'     for compassion and calibration 
func_get_diagonal_value <- function(x = ind_ft){
  m_dia <- readxl::read_excel(path = paste0(dir.eora_cleaned, ind_ft, '.xlsx')) %>%
    dplyr::filter(year %in% yrs) %>%
    dplyr::select(-year, -ctr) %>%
    tibble::column_to_rownames(var = 'iso3') %>%
    as.data.frame()
  
  m_diagonal <- diag(as.matrix(m_dia)) %>%
    as.data.frame() %>%
    tibble::rownames_to_column(var = 'iso3')%>%
    dplyr::rename('diag' = '.') %>%
    as.data.frame()
...
m_diagonal_data <- func_get_diagonal_value(x = ind_ft) %>%
    dplyr::mutate(diag = diag*coef_correction_curb_10e_issue_gdp)


### Outsourced footprints (oft) flow ---------------------------------------------------------------
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::mutate(net_in = net_in*coef_correction_curb_10e_issue_gdp) %>%
...
## subset data for viz
rel_not_merge4 <- rel_not_merge %>%
  dplyr::rename('value'     = 'gdp', 
                'value_rel' = 'gdp_rel', 'value_not' = 'gdp_not', 
                'value_dst' = 'gdp_dst', 'value_ner' = 'gdp_ner') %>%
  left_join(., m_diagonal_data, by = 'iso3') %>%
  as.data.frame()

rel_not_merge4 <- rel_not_merge4 %>%
  dplyr::filter(year == 2015)

p1 <- func_plot_netImports_vs_rel(df = rel_not_merge4)
p2 <- func_plot_netImports_vs_rel(df = rel_not_merge4, x = 'diag')
ggarrange(p1, p2)
...
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.025, lower_bound = lower_probs, upper_bound = upper_probs, direction = direction_sdg) %>%
...
func_plot_netImports_vs_rel(df = rel_not_merge)
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
cropland_area <- readxl::read_excel(path = xls) %>% 
  ...
  dplyr::mutate(cropland_area = cropland_area*1000) %>%   ## convert the unit to 'ha'
  as.data.frame()


### - Compare 'Arable land' data and  'Cropland_area' => the latter is better
ag_test <- merge(x = rel, y = cropland_area, by=c('iso3', 'year')) %>% dplyr::filter(year==2015)
function_scatter_plot(data = ag_test, x = 'value', y = 'cropland_area')
sum(is.na(ag_test)) 
sum(is.na(ag_test$cropland_area))
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  dplyr::mutate(net_in = net_in) %>%   
  spread(key = scenario, value = net_in)



### w/o trade, and with trade together  ------------------------------------------------------------
rel_not_merge <- cropland_area %>%
  dplyr::rename('value' = 'cropland_area') %>%
  merge(x = ., y = oft,         by = c('iso3', 'year'), all.x = T) %>% 
...
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0, 
  lower_bound = lower_probs, upper_bound = upper_probs, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
  dplyr::mutate(ind_sdg = ind_sdg) %>%
  as.data.frame()
...
total_Cereals <- total_w %>%
  ...
  dplyr::mutate(total_Cereals = total_Cereals*1000) %>%  ## ton to kg
  as.data.frame()
...
oft <- net_imports_bind  %>%
  ...
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  dplyr::mutate(net_in = net_in*1000) %>% ## ton to kg
  
  dplyr::mutate(
    net_in = case_when(    ## need adjustment
      net_in > 0 ~  (net_in)^0.5*net_in,
      net_in < 0 ~  (abs(net_in))^0.5*net_in,
      T ~ net_in),
    net_in = net_in/coef_correction_curb_10e_issue/1000
  ) %>% 
...
rel_not_merge <- total_Cereals %>%
  ...
  dplyr::mutate(
    ...
    value_dst = (value + fromDst)/pop,
    value_ner = (value + fromNer)/pop) %>%
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, value, everything())
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.025, trim.to.na = T,
  lower_bound = lower_probs, upper_bound = upper_probs, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
  dplyr::mutate(ind_sdg = ind_sdg) %>%
...
####  - 3.9.1 PM25/1000 person

  3.9.1 Mortality rate attributed to household and ambient air pollution 
    - Ton of PM25 per km2
    - Ton per 1000 person # updated on 8/18/2023
  
```{r}
ind_sdg    <- 'PM25_concentration' ## ton/km2 ==> ton per 1000 person 
ind_ft     <- 'PM25'
unit_check <- 10^3
direction_sdg <- -1

### Check the Unit!!!
### rel --> kt = 1000 ton = 10^6 kg
### oft --> ton
### total  --> km2
...
####  - 3.9.1s SO2 per 1000 person 

  3.9.1 Mortality rate attributed to household and ambient air pollution 
  - Ton of SO2 per km2
  - Ton of SO2 per 1000 person 

```{r}
ind_sdg  <- 'SO2 emissions per 1000 people' 
ind_ft     <- 'SO2'
unit_check <- 1000
direction_sdg <- -1
...
rel_not_check <- rel_not %>%
  dplyr::mutate(dif_rel_not = value_rel - value_not) %>%
  dplyr::select(ind, iso3, year, dif_rel_not, value_rel, value_not, everything())
func_plot_netImports_vs_rel(df = rel_not_merge)
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  # dplyr::mutate(net_in = net_in * coef_correction_curb_10e_issue) %>%
  spread(key = scenario, value = net_in)
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    value_rel =           (value)/pop*10^6,
    value_not = (value - fromAll)/pop*10^6,     ## few imports, few death
    value_dst = (value - fromNer)/pop*10^6,
    value_ner = (value - fromDst)/pop*10^6
    ) %>% 
  dplyr::select(ind, iso3, year, value, everything())
...
rel_not_merge_shp2 <- rel_not_merge_shp %>%
  dplyr::mutate(death_rate = fromAll/pop*10^6)
hist(rel_not_merge_shp2$death_rate, nclass = 100)


rel_not_merge_shp2 %>%
  dplyr::filter(!is.na(death_rate)) %>%
  tm_shape(.) +
    tm_borders() +
    tm_fill(col = "death_rate", palette = "RdYlBu", 
            title = 'net death_NCD_imports/million pop',
            # style = "fisher", 
            style = "cont", 
            breaks = c(-50, -10, 0, 0.12, 1.82, 6.76, 41), 
            n = 11, midpoint = 0) +
    tm_layout(main.title = "NCD death per million people due to red and processed meat trade in 2015")
...
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.025, trim.to.na = T,
  lower_bound = lower_probs, upper_bound = upper_probs, direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
  dplyr::mutate(ind_sdg = ind_sdg) %>%
  as.data.frame()
...
```{r}
ind_sdg    <- 'student_flow' ## global education person per 1000 population
ind_ft     <- 'student_flow'
unit_check <- 1
direction_sdg <- 1
...
###' Outsourced footprints (oft) flow --------------------------------------------------------------------
###' -- so here, we use outflow only - `total_ex`
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ...
    value_rel = fromAll*1000/pop,      ## more out-flow, the better
    value_not = fromAll*1000*0.1/pop, 
    ## 0 transfer under no-trade ## use 0.1 avoid 0, which cause problem in calculation later
    value_dst = fromDst*1000/pop,
    value_ner = fromNer*1000/pop,
    
    # value_rel = log(value_rel+ 0.001),      ## more out-flow, the better
    # value_not = log(value_not+ 0.001),
    # value_dst = log(value_dst+ 0.001),
    # value_ner = log(value_ner+ 0.001)
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'pop')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
...
###' Outsourced footprints (oft) flow --------------------------------------------------------------------
###' -- so here, we use outflow only - `total_in`
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    value_rel = fromAll,       ## more in-flow, the better
    value_not = fromAll*0.001, ## 0 transfer under no-trade ## use 0.1 avoid 0, which cause problem in calculation later
    value_dst = fromDst,
    value_ner = fromNer) %>%   
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, everything())
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0, trim.to.na = T,
  # lower_bound = lower_probs, upper_bound = upper_probs, 
  lower_bound = 0, upper_bound = 1, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
...
### Resource consumption or emissions under the current trading world situation -------------------------
# xls <- list.files(path = dir.input1, pattern = '^Total freshwater withdrawal', full.names = T); xls 
##' update to use this data; 8/18/2023
xls <- list.files(path = dir.input1, pattern = '^Annual freshwater withdrawals', full.names = T); xls 
rel_w <- readxl::read_excel(path = xls) %>% as.data.frame()  ## the status data
rel <- rel_w %>%
  dplyr::select(-ctr) %>%
  gather(key = 'year', value = 'value', `2000`:ncol(.)) %>%
...
### rel --> ton = 10^3 kg
### oft --> ton = 10^3 kg
### total water resources --> m3 (has been converted from billion m3 to m3 in the above chunk)
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.025, trim.to.na = F,
  # lower_bound = lower_probs, upper_bound = upper_probs, 
  lower_bound = 0, upper_bound = 1, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'total_ft')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::mutate(net_in = net_in/coef_correction_curb_10e_issue_energy) %>%  ## only for energy related indicators
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  spread(key = scenario, value = net_in)
...
pp <- func_plot_netImports_vs_rel(df = rel_not_merge)
# ggplotly(pp)
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.05, trim.to.na = T, 
  lower_bound = lower_probs, upper_bound = upper_probs, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
  dplyr::mutate(ind_sdg = ind_sdg) %>%
...
# score_7_1_2 <- df_score ## remove this indicator; 8/21/2023
...
####  - 7.3.1 total energy/gdp (Xu 2020 NatSus)

  7.3.1 Energy intensity measured in terms of primary energy and GDP (low energy intensity indicates high SDG ind_sdg score)
  
  - **Energy intensity** is expressed in `megajoules (MJ)` per unit of purchasing power parity GDP in constant 2017 USD figures. [SDG indicator metadata](https://unstats.un.org/sdgs/metadata/files/Metadata-07-03-01.pdf)
...
rel_tf <- readxl::read_excel(path = xls) %>% 
  as.data.frame()  %>%
  dplyr::select(-ctr) %>%
  gather(key = 'year', value = 'value', `2000`:ncol(.)) %>%
  dplyr::mutate(value = value) %>%  
...
###' - Total Primary energy consumption_TJ
...
###' 
rel_comp <- rel_tf %>%
  left_join(., rel, by = c('iso3', 'year'))
function_scatter_plot(data = rel_comp, x = 'value.x', y = 'value.y', log.xy = T)
###' --> almost the same 
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  dplyr::mutate(net_in = net_in/coef_correction_curb_10e_issue_energy*10) %>%  ## only for energy related indicators
  spread(key = scenario, value = net_in) %>%
  as.data.frame()
# unique(oft$scenario)
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    value_rel = (value +       0)*10^6/gdp_rel, ## TJ = 10^6 MJ
    value_not = (value + fromAll)*10^6/gdp_not,
    value_dst = (value + fromNer)*10^6/gdp_dst,
    value_ner = (value + fromDst)*10^6/gdp_ner
    ) %>%
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, value, everything())

rel_not_merge1 <- rel_not_merge %>%
  left_join(., m_diagonal_data, by = 'iso3') %>%
  dplyr::mutate(diag = diag/coef_correction_curb_10e_issue_energy*10) %>%
  dplyr::mutate(chg = value_rel - value_not) %>%
  as.data.frame()
...
func_plot_netImports_vs_rel(df = rel_not_merge1, x = 'diag')
func_plot_netImports_vs_rel(df = rel_not_merge)
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
...
####  - 7.2.1 renewable/total (Xu 2020 NatSus)
...
func_plot_netImports_vs_rel(df = rel_not_merge)
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'EnergyTotal_rel')
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'EnergyTotal_rel', y = 'value')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
base <- gdp_rel_not %>% 
  ...
  #   base_rel = base,
  #   base_not = base - base,
  #   base_dst = gdp_dst - base,
  #   base_ner = gdp_ner - base
  #   ) %>%
  as.data.frame()
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'base', y = 'gdp_rel')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::mutate(net_in = net_in/coef_correction_curb_10e_issue_material*10) %>% 
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  spread(key = scenario, value = net_in)
...
#### - 9.4.1x CO2 emissions per unit of value added
  ...
  This is measured as the kilograms of CO₂ emitted per dollar of GDP. Emissions include fossil fuel and industry emissions. https://ourworldindata.org/grapher/co2-intensity 

```{r}
ind_sdg  <- 'co2_per_gdp' ## kg/USD
ind_ft   <- 'CO2'
direction_sdg <- -1
...
rel <- readxl::read_excel(path = xls) %>%  
  dplyr::select(-ctr) %>%
  gather(key = 'year', value = 'value', `2000`:ncol(.)) %>%
  dplyr::mutate(value = value*10^6) %>%  ## kt = 10^6 kg
  as.data.frame()
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  dplyr::mutate(net_in = net_in*10^3) %>%  ## ton = 10^3 kg
  # dplyr::mutate(net_in = net_in*coef_correction_curb_10e_issue/10) %>%
  spread(key = scenario, value = net_in)
...
####  - 9.5.2a Researchers per million - Inventor migration

  9.5.2 Researchers (in full-time equivalent) per million inhabitants 
  
  - Inventor migration
  - Global talent flows (High-skilled immigrants)

```{r}
ind_sdg  <- 'Researchers per million people - Inventor'  ## person per 10^6 people
ind_ft     <- 'Inventor migration_matrix'                ## person
unit_check <- 10^6
direction_sdg <- 1

### Check the Unit!!!
### rel -->          ## Researchers in R&D (per million people)
### oft --> person
### pop --> person
...
###
###' Resource consumption or emissions under the current trading world situation ----------
###' --> get needed data via `10_country_profileData_prepare.Rmd`
xls <- list.files(path = dir.input1, pattern = '^Researchers in', full.names = T); xls 
rel_w <- readxl::read_excel(path = xls) %>% as.data.frame()  ## the status data
rel <- rel_w %>%
  dplyr::select(-ctr) %>%
  gather(key = 'year', value = 'value', `2000`:ncol(.)) %>%
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    value_rel = (value - 0),
    value_not = (value - fromAll/(pop/10^6)),
    value_dst = (value - fromNer/(pop/10^6)),
    value_ner = (value - fromDst/(pop/10^6))
    ) %>%  
  ### for easy-checking the numbers
...
###' normalization  ----------------------------------------------------------------------
###' Be careful with **Data Direction!**   <-- `direction_sdg`
...
score_9_5_2a <- df_score
func_plot_sdg_ScoreChange_byGroup(df_score = df_score, bar_or_violin = 'both')
```



####  - 9.5.2b Researchers per million - High-skilled immigrants
...
```{r}
ind_sdg  <- 'Researchers per million people - High-skilled'  ## person per 10^6 people
ind_ft     <- 'High-skilled immigrants_matrix'               ## person
unit_check <- 1
direction_sdg <- 1

### Check the Unit!!!
### rel -->          ## Enrolment in tertiary education, all programmes, both sexes (number)
### oft --> person   ## number
### pop --> person
...
###' Resource consumption or emissions under the current trading world situation ----------
###' --> get needed data via `10_country_profileData_prepare.Rmd`
xls <- list.files(path = dir.input1, pattern = '^Enrolment in tertiary education', full.names = T); xls 
rel_w <- readxl::read_excel(path = xls) %>% as.data.frame()  ## the status data
rel <- rel_w %>%
  dplyr::select(-ctr) %>%
  gather(key = 'year', value = 'value', `2000`:ncol(.)) %>%
...
### w/o trade, and with trade together  -----------------------------------------------------------------
rel_not_merge <- rel %>%
  merge(x = ., y = oft,         by = c('iso3', 'year'), all.y = T) %>% 
  merge(x = ., y = pop_l,       by = c('iso3', 'year'), all.x = T) %>%
  dplyr::filter(year %in% yrs) %>%
  dplyr::mutate(
    ind       = unique(oft$ind),
    value_rel = (value)/(pop/10^6),
    value_not = (value_rel - fromAll/(pop/10^6)),
    value_dst = (value_rel - fromNer/(pop/10^6)),
    value_ner = (value_rel - fromDst/(pop/10^6))
    ) %>%  
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, value, everything())
...
score_9_5_2b <- df_score
func_plot_sdg_ScoreChange_byGroup(df_score = df_score, bar_or_violin = 'violin')
```
...
ind_ft     <- 'ODA_DACSECTOR_oecd_Infrastructure_matrix'
unit_check <- ''
direction_sdg <- 1
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ...
    value_not = ifelse( (-fromAll) > 0, (-fromAll)*0.01, 0+1),  ## under no-trade
    ...
    ##' option 2
    ##'   inflow can also be helpful for infrastructure development, 
    ##'   e.g., in developing countries
    # value_rel = abs(-fromAll),       ## total outflow as the indicator
    # value_not = abs(-fromAll)*0.01,  ## to avoid extreme values, this assumes the total outflow become 1% of before 
    # value_dst = abs(-fromDst),       ## total outflow to distant regions
    # value_ner = abs(-fromNer),
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'gdp_rel')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.1, trim.to.na = T,
  lower_bound = lower_probs, upper_bound = upper_probs,
  # lower_bound = 0, upper_bound = 1, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'value_lab')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
#### - 10.7.4 Refugees proportionby country of origin

  10.7.4 Proportion of the population who are refugees, by country of origin

```{r}

ind_sdg    <- 'Refugees proportion' ## % population
ind_ft     <- 'Refugee_matrix'
unit_check <- 1
direction_sdg <- -1
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ind       = ind_ft,
    ## the evaluation here is the same approach used in 16.2.2
    victims   = 0 - fromAll, ## the number of victims from the country of origin
    value_rel = (victims)             /pop*10^6,  ## victims per 100,000 population
    value_not = (victims - (-fromAll))/pop*10^6,  ## 0 transfer under no-trade
    value_dst = (victims - (-fromNer))/pop*10^6,
    value_ner = (victims - (-fromDst))/pop*10^6
    ) %>%   
...
#' 
f <- paste0(dir.eora_cleaned, ind_ft, '.xlsx')
rel_outflow_imf <- readxl::read_excel(f) %>%
...
rel_check <- merge(
  ...
  y = rel_outflow_imf %>% dplyr::rename('fdi_outflow' = 'value'), 
  by = c('iso3', 'year')
)
...
#'    The two data are very different ... not sure why. 
#'    But *FDI's bilateral data make more sense*. 
  

### w/o trade, and with trade together  -----------------------------------------------------------------
rel_not_merge <- rel_outflow_imf %>%
  merge(x = ., 
        y = oft,   
  ...
  dplyr::mutate(
    ...
    ##' option 1
    ##'    if a country's net outflow < 0, we assume it does not have FDI, 
    ##'    and assign 0 as its net outflow
    ...
    ##' option 2
    ##'   it would be better to keep using the data to indicate that if outflow > 0, a country 
    ##'   positively contribute to SDG, while outflow < 0 means a country negatively contribute to SDGs
    ##'   In this way can avoid extreme values like 0 or 1, which are substantially smaller than other data values
    value_rel = value,  ##
    value_not = value-(-fromAll), 
    value_dst = (value-(-fromNer)), ## (total outflow) - (nearby outflow)
    value_ner = (value-(-fromDst)), ## (total outflow) - (distant outflow)
...
#### - 10.b.1z PI for development

  10.b.1 Total resource flows for development, by recipient and donor countries and type of flow (e.g. official development assistance, foreign direct investment and other flows) 

```{r}

ind_sdg  <- 'PI for development' ## 
ind_ft   <- 'CPIS_matrix'
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ...
    ##'    and assign 0 as its net outflow
    # value     = fromAll,
    # value_rel = ifelse(value > 0, value, 0+1)/gdp_rel,  ##
    # value_not = (0+1)/gdp_rel,                          ## under no-trade ## add $1 to avoid zero values
    # value_dst = ifelse( (value-(-fromNer)) >0, (value-(-fromNer)), 0+1)/gdp_rel, ## (total outflow) - (nearby outflow)
    # value_ner = ifelse( (value-(-fromDst)) >0, (value-(-fromDst)), 0+1)/gdp_rel  ## (total outflow) - (distant outflow)
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0.01, trim.to.na = T,
  lower_bound = lower_probs, upper_bound = upper_probs, 
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
  dplyr::mutate(ind_sdg = ind_sdg) %>%
...
### SDG 13 
  ...
  13.2 Take urgent measures to combat climate change and its impacts
  
#### - 13.2.2 total GHG

  * [13.2.2 Total greenhouse gas emissions per year](https://unstats.un.org/sdgs/metadata/files/Metadata-13-02-02.pdf)
    Unit of measure (UNIT_MEASURE): Mt CO2-equivalent
    A metric megaton is a unit of mass equal to 10^6 (one million) kilograms. 
```{r}

ind_sdg    <- 'GHG emissions per year' ## Mt/yr
ind_ft     <- 'GHG' ## ton
direction_sdg <- -1
...
rel <- rel_w %>%
  dplyr::select(-ctr) %>%
  gather(key = 'year', value = 'value', `2000`:ncol(.)) %>%
  dplyr::mutate(value = value/1000) %>%  ## kt = 1/1000 Mt
  as.data.frame()
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  dplyr::mutate(net_in = net_in/10^6) %>%  ## ton = 1/10^6 Mt 
  dplyr::mutate(net_in = net_in*coef_correction_curb_10e_issue/10) %>%  ## adjustment
  spread(key = scenario, value = net_in)
...
### big - bad?    --> func_norm_sdg_score_bad
df_score <- func_norm_sdg_score_auto(
  df = rel_not, trim = 0, trim.to.na = F,
  # lower_bound = lower_probs, upper_bound = upper_probs, 
  lower_bound = 0, upper_bound = 1,
  direction = direction_sdg) %>%
  # spread(key = 'scenario', value = c('value_norm')) %>%
...
####  - 13.2.s co2/forest
  ...
  **13.2.s** CO2 emissions intensity of areas under forest management (GtCO2-equivalent per ha)
 
  - Based on Xu et al 2020 NS paper. 
  - low value indicates high SDG target score
  - S  = f(CE / FA);        # CE: CO2 emissions, FA: forest area
  - S' = f((CE−NEC) / FA)
  
```{r}
ind_sdg    <- 'CO2 per area of forest' ## ton/km2
ind_ft     <- 'Landuse_forest'         ## ha
unit_check <- 10^(-2) 
direction_sdg <- -1

### Check the Unit!!!
### rel --> km2                  (forest area data from world bank)
### oft --> ha  = 0.01 km2       (embodied forest area from Eora)
### co2 --> ton


###' Resource consumption or emissions under the current trading world situation ------------------------
###' Forest area (sq. km)_4yrs.xlsx
###' 
...
oft <- net_imports_bind  %>%
  ...
  dplyr::mutate(
    net_in = case_when(    ## need adjustment
      net_in > 0 ~       (net_in)^exp_n*net_in,
      net_in < 0 ~  (abs(net_in))^exp_n*net_in,
      T ~ net_in),
    net_in = net_in/coef_correction_curb_10e_issue/10
  ) %>% 
  spread(key = scenario, value = net_in)
...
forest_rel_not <- rel %>%
  ...
  dplyr::mutate(
    ind     = unique(oft$ind),
    for_rel = (value +      0),
    for_not = (value - fromAll), ## if no trade, a net importer would have less forest 
    for_dst = (value - fromNer),
    for_ner = (value - fromDst)
    ) %>% 
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, for_rel, for_not, for_dst, for_ner, everything()) %>%
  dplyr::select(-value)
```
...
###' Resource consumption or emissions under the current trading world situation -------------------------
###' 
###' --> Nutrient nitrogen N (total)_Use per area of cropland_kg_ha
###' 
xls <- list.files(path = dir.input1, pattern = '^Nutrient nitrogen N', full.names = T); xls 
rel <- readxl::read_excel(path = xls) %>% 
  dplyr::select(-ctr) %>%
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ind       = ind_ft,
    N_used    = value*cropland_area,      ## kg/ha * ha = kg
    value_rel = (N_used + 0)      /cropland_area,   ## kg/ha -- Nutrient nitrogen N (total)_Use per area of cropland_kg_ha
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'N_used')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
### Resource consumption or emissions under the current trading world situation -------------------------
### --> Nutrient phosphate P2O5 (total)_Use per area of cropland_kg_ha
...
oft <- net_imports_bind  %>%
  dplyr::filter(ind == ind_ft) %>%
  dplyr::mutate(net_in = net_in*10^6/coef_correction_curb_10e_issue_P) %>%  ## 1 kton = 10^6 kg
  dplyr::select(ind, year, iso3, net_in, scenario) %>%
  spread(key = scenario, value = net_in)
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'P_used')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
###' Resource consumption or emissions under the current trading world situation ---------
###'    Fisheries production - marine capture in tons
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    value     = ifelse(value ==0, NA, value),## for countries without fisheries, set them as NA
...
###' option 1
###' 
# rel_not_merge <- rel_forest %>%
#   merge(x = ., y = oft,         by = c('iso3', 'year'), all.x = T) %>%
#   merge(x = ., y = land_total,  by = c('iso3', 'year'), all.x = T) %>%
#   dplyr::filter(year %in% yrs) %>%
#   dplyr::mutate(
#     ind       = ind_ft,
#     value_rel = (rel_forest)        /(land_total_km2 * 100),      ## forest area/ total land area
#     value_not = (rel_forest-fromAll)/(land_total_km2 * 100),
#     value_dst = (rel_forest-fromNer)/(land_total_km2 * 100),
#     value_ner = (rel_forest-fromDst)/(land_total_km2 * 100),
#     ) %>%  ## total land area does not change
#   ### for easy-checking the numbers
#   dplyr::select(ind, iso3, year, everything()) 


###' option 2 - the results are the same as option 1
rel_not_merge2 <- rel_forest %>%
  merge(x = ., y = forest_rel_not, by = c('iso3', 'year'), all.x = T) %>%
  merge(x = ., y = land_total,  by = c('iso3', 'year'), all.x = T) %>%
  dplyr::filter(year %in% yrs) %>%
  dplyr::mutate(
    ind       = ind_ft,
    value_rel = for_rel/(land_total_km2 * 100),      ## forest area/ total land area
    value_not = for_not/(land_total_km2 * 100),
    value_dst = for_dst/(land_total_km2 * 100),
    value_ner = for_ner/(land_total_km2 * 100)
    
    ) %>%  ## total land area does not change
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, everything()) %>%
  dplyr::select(-rel_forest)

rel_not_merge3 <- rel_not_merge2 %>%
  dplyr::mutate(net_imports = for_rel - for_not)


# rel_not <- rel_not_merge %>%
rel_not <- rel_not_merge2 %>%
  dplyr::select(ind, iso3, year, value_rel, value_not, value_dst, value_ner)
...
# func_plot_netImports_vs_rel(df = rel_not_merge,  x = 'rel_forest')
func_plot_netImports_vs_rel(df = rel_not_merge3, x = 'for_rel', y = 'net_imports')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
  ...
  - forest change = (forest_t - forest_0)/forest_0
    ...
    S  = f( (FA_t − FA_base) / FA_base)
    S∗ = f( (FA_t + NEVF_t − FA_base) / FA_base)
    https://www.nature.com/articles/s41893-020-0572-z/tables/1
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ind       = ind_ft,
    value_rel = (for_rel - base)/base,
    value_not = (for_not - base)/base,
    value_dst = (for_dst - base)/base,
    value_ner = (for_ner - base)/base
    ) %>%
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, value, everything())
...
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'for_rel')
func_plot_netImports_vs_rel(df = rel_not_merge, x = 'for_rel', y = 'base')
func_plot_sdg_ScoreChange_byGroup(df_score = df_score)
```
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    arms_exports = 0 - fromAll,
    value_rel = (arms_exports  + 1),  ## net exports as the measurement, add 1 to avoid 0 that may cause error in calculation
    value_not = (arms_exports - (-fromAll)),  ## 0 transfer under no-trade, add 0.1 to avoid 0 that may cause error in calculation
    value_dst = (arms_exports - (-fromNer)),
    value_ner = (arms_exports - (-fromDst))
  ) %>%
  dplyr::select(-arms_exports) %>%
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, everything())
...
###'
###'
###'See section `10.b.1x` for updated `rel` data from IMF
...
rel_not_merge <- rel_outflow_imf %>%
  ...
  dplyr::mutate(
    ...
    ##' option 2
    ##'   see details in section for `10.b.1x`
    value_rel = value,  ##
    value_not = value-(-fromAll), 
    value_dst = (value-(-fromNer)), ## (total outflow) - (nearby outflow)
    value_ner = (value-(-fromDst)), ## (total outflow) - (distant outflow)
...
### Resource consumption or emissions under the current trading world situation -------------------------
###' --> Net ODA and official aid received (constant 2020 US$)
###'    !!! This data have many misisng values
...
rel_not_merge <- rel %>%
  ...
  dplyr::mutate(
    ...
    ## use the WB data to fill the data gap here
    fromAll = ifelse(fromAll %in% c(0, NA), value, fromAll),
    ...
    ##' option 2 
    # value_rel = (-fromAll)           /gdp_rel, ## total outflow as the indicator
    # value_not = (-value - (-fromAll))/gdp_not, 
    # value_dst = (-value - (-fromNer))/gdp_dst, ## total outflow to distant regions
    # value_ner = (-value - (-fromDst))/gdp_ner,
    
    ##' option 3
    ##'   since this indicator underline 'cooperation', the absolute value should be used.
    # value_rel = abs(-fromAll)               /gdp_rel, ## total outflow as the indicator
    # value_not = abs((-fromAll)*0.01)        /gdp_not, 
    # value_dst = abs((-fromAll) - (-fromNer))/gdp_dst, ## total outflow to distant regions
    # value_ner = abs((-fromAll) - (-fromDst))/gdp_ner,
    
    ##' option 4 - see section `9.a.1`
    ##'   since this indicator underline 'cooperation', the absolute value should be used.
    value_rel = abs(-fromAll)/gdp_rel, ## total outflow as the indicator
    value_not = abs(-fromAll)*0.01/gdp_not, 
    value_dst = abs(-fromNer)/gdp_dst, ## total outflow to distant regions
    value_ner = abs(-fromDst)/gdp_ner,
...
#### - 17.3.2 Remittances per GDP

  17.3.2 Volume of remittances (in United States dollars) as a proportion of total GDP
  
  Definition: Personal remittances received as proportion of GDP is the *inflow* of personal remittances expressed as a
percentage of Gross Domestic Product (GDP). [See UNSD Metadata](https://unstats.un.org/sdgs/metadata/files/Metadata-17-03-02.pdf)
...
```{r}
ind_sdg  <- 'Remittances per GDP' ## 
ind_ft   <- 'Remittance_matrix'
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    value     = fromAll,            ## received more, the better
    value_rel = value,              ##
    value_not = (value - fromAll)/gdp_rel, 
    value_dst = (value - fromNer)/gdp_rel,  
    value_ner = (value - fromDst)/gdp_rel
    ) %>%
  ### for easy-checking the numbers
  dplyr::select(ind, iso3, year, value, everything())
...
#### - 17.6 Access to innovation - Inventor flow
...
```{r}
ind_sdg    <- 'Access to innovation - Inventor flow'  ## more Inventor inflow the better for SDG
ind_ft     <- 'Inventor migration_matrix'   ## person
unit_check <- ""
direction_sdg <- 1
...
rel_not_merge <- oft %>%
  ...
  dplyr::mutate(
    ind       = unique(oft$ind),
    ##' The more inventor inflow, the better for innovation in the receiving country 
    ##' The more inventor outflow, the worse for innovation in the sending country (`brain drain`)    
    value     = fromAll,  
    value_rel = (value -        0)/(pop/10^6),
    value_not = (value - (fromAll)/(pop/10^6)),
    value_dst = (value - (fromNer)/(pop/10^6)),
    value_ner = (value - (fromDst)/(pop/10^6))
    ) %>%  
