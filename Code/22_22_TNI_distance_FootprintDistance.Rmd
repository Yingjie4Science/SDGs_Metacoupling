...
for (ind in unique(ft_ind$ind)) {
  ...
  ### --> if we want to only see if the interaction happen acorss large income difference groups
  links_dist_type2 <- links_dist %>%
    dplyr::mutate(
      od_direction = case_when(
  ...
  if (n_flows == 2) {
    links_dist_type <- links_dist_type2
  } else if (n_flows == 3) {
    links_dist_type <- links_dist_type3
  } else {
    print('NA ...................')
    break
  }
...
p1 <- ft_d %>% 
  ...
  ylim(0,20) + 
  scale_x_continuous(breaks = c(seq(0, d_max, 8), round(mean(ft_d$d_avg_ft, na.rm=T), digits = 1))) +
  xlab(label = bquote('Distance (10'^3~'km)')) + 
  labs(fill='') # Impact pairs


ggarrange(p1, labels = '')
fname <- paste0(dir.fig, '01main/', 'ImpactDistance_by_ft',
                '_class', od_direction_count, '_', today, '.jpg'); fname
ggsave(filename = fname, plot = last_plot(), width = 6.5+0.5, height = 9, units = 'in', dpi = 300)
```
...
```{r plot - summary across indicators}
ind <- 'AvgAcrossFT'
ft_d_summary <- ft_d %>%
...
fname <- paste0(dir.fig, '01main/', 'ImpactDistance_', ind,
                '_class', od_direction_count, '_', today, '.jpg'); fname
