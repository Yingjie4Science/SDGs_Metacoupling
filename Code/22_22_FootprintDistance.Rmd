...
cat('\n', length(unique(ft_ind$ind)), 'indicators were included in the analysis.\n')
```
...
for (ind in unique(ft_ind$ind)) {
  ...
  ##' --> If iqr == 0, `robust_scaling` will not work as iqr will be used as the denominator
  ##'     in the function.
  ##' --> To address the issue, we will use Z-Score Scaling when iqr == 0.
  which_scaling <- IQR(links$weight, na.rm = TRUE)
  if (which_scaling == 0) {
    links_w <- func_z_scaling(data = links, var = 'weight') %>%
      dplyr::mutate(weight = func_norm_minmax(weight, a=0.01, b=100))
  } else {
    links_w <- func_robust_scaling(data = links, var = 'weight') %>%
      dplyr::mutate(weight = func_norm_minmax(weight, a=0.01, b=100))
  }
  
  links_dist <- 
    merge(x = links_w, y = dist,   
          by.x = c('from', 'to'), by.y = c('iso_o', 'iso_d'), all.x = T) %>%
    merge(x = ., y = grp_id, 
          by.x = 'from', by.y = 'iso_a3', all.x = T) %>% dplyr::rename('group_from' = 'group_id') %>%
    merge(x = ., y = grp_id, 
          by.x = 'to',   by.y = 'iso_a3', all.x = T) %>% dplyr::rename('group_to' = 'group_id') %>%
...
```{r plot fig ex6 - by ft indicator}
...
```{r plot fig 2d - summary across indicators}
ind <- 'AvgAcrossFT'
