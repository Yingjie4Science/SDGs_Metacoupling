...
ctr_wb <- wb(indicator = 'SP.POP.TOTL', country = "countries_only") %>%
  distinct(iso3c, iso2c, country)  ## 216 countries in the WB list
ctr_wb <- merge(ctr_wb, wb_geo, 
                by = c('iso3c','country','iso2c'), all.x = T)%>%
  select(country, iso3c, region, income)
...
ctr <- ctr0 %>%
  mutate(income = ifelse(iso3_eora=='SWZ' & is.na(income), 'Lower middle income', income), ## replace(income, iso3_eora=='SWZ', 'Lower middle income'),
...
```{r FAO country code}
dir.fao <- './Data/data_01_raw/FAOSTAT/'
...
ctr.fao <- readxl::read_excel('./Data/data_01_raw/FAOSTAT/Country codes_names_FAO_Revised_by_Li.xlsx') %>%
...
ctr.faostat <- readr::read_csv("./Data/data_01_raw/FAOSTAT/FAOSTAT_Country_Region.csv") %>%
...
unique(fao_dataset$item)
unique(fao_dataset$element)
...
df <- dt %>%
  ...
  as.data.frame() 
names(df)  
iname <- unique(df$indicator)[!is.na(unique(dfwb$indicator))]; iname
### 2. fill NA
function_fill_na_wb(df)
### 3. get a cleaned data and save to dir
...
csv <- paste0(dir.fao, 'Inputs_Pesticides_Trade/', csv); csv
...
### 5, save to results
fname <- paste0(dir.process, 'cleaned/', iname, '_4yrs.xlsx'); fname
...
getwd()
dir.gbd <- './Data/data_01_raw/GBD 2016 Health-related SDGs Indicators 1990-2030 Scaled and Unscaled Values/'
...
ind <- '1.1_TOTAL.FINAL.ENERGY.CONSUM'
### 1. download data from WB
dfwb  <- wb(indicator = ind, country = "countries_only",
            startdate = yr_start, enddate = yr_end) %>%
  as.data.frame() %>%
...
wk_compare %>%
  ...
  geom_point(aes(x = wk1, y = wk3), color = 'red',  alpha = 0.3, size = 5) +
  geom_point(aes(x = wk1, y = wk4), color = 'blue', alpha = 0.3, size = 2) +
...
dt1 <- dt0 %>%
  dplyr::select(-source, -source.label, -indicator, -indicator.label, 
...
dir.plus <- './Data/data_02_intermediate/dt02_flows/Global Material Flows Database_unep_irp/'

### Resource consumption or emissions under the current trading world situation -------------------------
xls <- paste0(path = dir.plus, 'DE.csv'); xls 
...
xls <- paste0(path = dir.plus, 'MF.csv'); xls 
...
csv <- 'UNdata_Total wastewater generated.csv' ## only 31 countries
...
```{r - fill na, eval=FALSE, include=FALSE}
### fill na
df <- oecd
