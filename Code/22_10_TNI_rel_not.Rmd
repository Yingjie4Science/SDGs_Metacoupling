...
map1 <- spillover_index_shp %>%
  ...
  scale_fill_distiller(palette = palette_map, direction = palette_drc, na.value = 'black', name = 'Transnational\nImpact\nIndex') +
...
spillover_index_shp %>%
  ...
  tm_fill(col = "index", title = 'Transnational\nImpact\nIndex', 
          ...
          breaks = c(index_min, seq(-30, 30, 10), index_max), style = "cont", 
          legend.reverse = T,
          colorNA = 'black') +
  tm_layout(
    title = 'b', 
...
p <- spillover_index_top_bottom %>%
  ...
  annotation_custom(grob = grid::textGrob(
    ...
    gp=gpar(col="#d8b365", cex=.6)),
    xmin = 0, xmax = 0.1, ymin = 10) +
  annotation_custom(grob = linesGrob(
    arrow=arrow(type="open", ends="first", length=unit(2,"mm")), 
    gp=gpar(col="#5ab4ac", lwd=2)), 
    xmin = 0, xmax = 0, ymin = 0, ymax = -20) +
  annotation_custom(grob = grid::textGrob(
    label = "Impacted", hjust=4, 
    gp=gpar(col="#5ab4ac", cex=.6)),
    xmin = 0, xmax = 0.1, ymin = -30) +
...
ggplot(spillover_index_type_SDGtype, 
  ...
  theme(
    ...
    legend.margin=margin(0, 0, 0, 0)) +
  ylab('Transnational Impact Index') + xlab('Country')

fname <- paste0(dir.fig, '01main/', 'ex_4_spillover_index_top_bottom_ByType_', postfix, '_', today, '.jpg'); fname
ggsave(filename = fname, plot = ggplot2::last_plot(), width = width_2col/3*2, height = width_2col/2, units = 'cm', dpi = 600)
```
