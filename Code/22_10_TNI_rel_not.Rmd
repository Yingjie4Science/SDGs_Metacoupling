...
---
  ...
  1. Load the combined EORA indicator data (4 years data in one table), and calculate `net imports` for each indicator, each year, and each country. 
...
for (i in 1:nrow(direction_all)) {
  ...
  print( paste(ind, drt, in_out, sep = '    ') ) 
...
ggplot() + 
  geom_point(aes(x = net_imp_con_i$net_in, y = net_imp_pro_i$net_in)) + 
  geom_abline(slope = 1, color = 'red') +
  ggtitle(paste0(ind_i, ' - net imports')) +
  xlab('con-based') + 
  ylab('pro-based') +
...
### add "index" as a column of `spillover_norm`
spillover_norm_index <- spillover_norm %>%
  left_join(., spillover_index, by = c('iso3', 'year')) %>%
  dplyr::select(index, everything())

### Rank each column
# Identify numeric columns
spillover_norm_index_ranked <- spillover_norm_index
numeric_columns <- sapply(spillover_norm_index_ranked, is.numeric)
spillover_norm_index_ranked[numeric_columns] <- lapply(spillover_norm_index_ranked[numeric_columns], rank)
write_xlsx(x = spillover_norm_index_ranked, 
           path = paste0(dir_results, 'results_check/spillover_norm_index_ranked.xlsx'))
...
spillover_index_rank <- spillover_index_shp %>%
  st_drop_geometry() %>%
  dplyr::filter(year == 2015) %>%
  arrange(index) %>%
...
spillover_index_top_bottom <- spillover_index_rank %>%
...
p <- spillover_index_top_bottom %>%
  ...
  annotation_custom(grob = linesGrob(
    ...
    xmin = 0, xmax = 0, ymin = 0, ymax = 35) + ## 40
  annotation_custom(grob = grid::textGrob(
    label = "Impacting", hjust=-0.2, ## 0; negative value -> move right
    gp=gpar(col="#d8b365", cex=.7)),
    xmin = 0, xmax = 0.1, ymin = 10) +
  annotation_custom(grob = linesGrob(
    arrow=arrow(type="open", ends="first", length=unit(2,"mm")), 
    gp=gpar(col="#5ab4ac", lwd=2)), 
    xmin = 0, xmax = 0, ymin = 0, ymax = -20) + ## -25
  annotation_custom(grob = grid::textGrob(
    label = "Impacted", hjust=3.6, ## larger positive value -> move left
...
##' note: this figure will be combined with Figure ex_4, so 
##' there is no need to save this as a separate figure
```
...
```{r ... figEx_4 - Index rank with MC categories}
...
```{r ... figEx5 - score map of each impact}
