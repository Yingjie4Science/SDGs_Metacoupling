...
# Analysis
  ...
  **Spillover Score**: the normalized score of each spillover, which measured by "net imports". 
  Scores should be interpreted in the same way as the SDG Index score: from 
    - `0   (poor performance)`, i.e., significant negative spillovers 
...
fname <- paste0(dir.flowScenario, 'net_imports_fromAll_', n_ft_ind, '.RData'); fname
...
```
  ...
  To check on each country's TNI impact (spillover impact)
...
#   dplyr::select(ind, iso3, year, score) %>%
#   dplyr::filter(year == 2015) %>%
#   ggplot(., aes(y = iso3, x = ind)) +                           # Create heatmap with ggplot2
#   geom_tile(aes(fill = score))
# ggp      


library(pheatmap)
f <- paste0(dir.fig, 'spillover score by TNI by country.png')
...
fignm <- paste0('fig2b_map_spillover_index_2015_', today, '.jpg')
...
fname <- paste0(dir.fig, 'spillover/', 'fig2a_spillover_index_top_bottom_color_country2group_', today ,'.jpg'); fname
...
fname <- paste0(dir.fig, 'spillover/', 'spillover_index_top_bottom_ByType_', today, '.jpg'); fname
ggsave(filename = fname, plot = last_plot(), width = width_2col/3*2, height = width_2col/2, units = 'cm', dpi = 600)
```
