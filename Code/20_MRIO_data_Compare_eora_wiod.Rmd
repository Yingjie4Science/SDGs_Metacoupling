...
dir.eora    <- './data_02_intermediate/dt02_flows/eora/'
dir.fig     <- './Figure/'
...
# xls.name.eora <- 'YJ_Energytotal_2005.xlsx'; unit_name <- 'PJ'         ## PJ = 1000 TJ
...
path_dt_wiod <- 'G:/My Drive/_paper/SDGs/SDGs_Global_Trade/data/update_0503_SUM_dist/'
...
xls_iso_eora <- "./_eora_190country_iso3_match_update.xlsx"
xls_iso_wiod <- "./_wiod_41country_iso3.xlsx"
...
col_name_ctr <- names(df.eora1)[-c(1, ncol(df.eora1))]; col_name_ctr ## remove "ctr" in the 1st col and "iso3" in the last
...
countries <- colnames(s2[1:41]); countries
...
df.wiod2005 <- df.wiod1 %>% as.data.frame() %>%
  dplyr::select(year, ctr, iso3, countries) %>%
  dplyr::filter(year == 2005)

names(df.wiod2005)
unique(df.wiod2005$ctr)
ctr_ls_wiod <- unique(df.wiod1$iso3); ctr_ls_wiod
### rename the cols
...
df_common_ctr_wiod <- df.wiod2005 %>%
  ...
  dplyr::select(iso3, ctr_ls_common) %>%
  arrange(iso3)
df_common_ctr_eora <- df.eora1 %>%
  dplyr::filter(iso3 %in% ctr_ls_common) %>%
  dplyr::select(iso3, ctr_ls_common) %>%
  arrange(iso3)
