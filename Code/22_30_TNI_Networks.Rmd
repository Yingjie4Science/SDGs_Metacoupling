...
nodes <- 
  ...
  merge(x = ., y = grp, by.x = 'iso3', by.y = 'iso_a3', all.x = T) %>%
  # dplyr::filter(name != 'N. Cyprus') %>%
  distinct(iso3, .keep_all = T) %>%
  dplyr::mutate(#inc_group = ifelse(Id == 'ANT' & is.na(inc_group), '1. High income', inc_group),
                # inc_group = ifelse(Id == 'GIB' & is.na(inc_group), '1. High income', inc_group),
                inc_group = ifelse(iso3 == 'ROW' & is.na(inc_group), '4. Low income', inc_group),
                inc_group = ifelse(iso3 == 'SUN' & is.na(inc_group), '4. Low income', inc_group),
                # inc_group = ifelse(Id == 'TKL' & is.na(inc_group), '4. Low income', inc_group),
                # inc_group = ifelse(Id == 'TUV' & is.na(inc_group), '2. Upper middle income', inc_group),
                ) %>%
  # dplyr::select(-inc_group) #%>%
  dplyr::mutate(inc_group = as.factor(inc_group)) %>%
  merge(., pop %>% dplyr::select(iso3, `2015`), by.x = 'iso3', by.y = 'iso3', all.x = T) %>%
  arrange(iso3) %>%
  separate(col = eco_group, into = c('node_type', 'type.label')) %>%
...
nodes <-
  ...
  merge(x = ., y = grp, by.x = 'iso3', by.y = 'iso_a3', all.x = T) %>%
  distinct(iso3, .keep_all = T) %>%
  dplyr::mutate(inc_group = ifelse(iso3 == 'ROW' & is.na(inc_group), '4. Low income', inc_group),
                inc_group = ifelse(iso3 == 'SUN' & is.na(inc_group), '4. Low income', inc_group)) %>%
  dplyr::mutate(inc_group = as.factor(inc_group)) %>%
  merge(., pop %>% dplyr::select(iso3, `2015`), by.x = 'iso3', by.y = 'iso3', all.x = T) %>%
  arrange(iso3) %>%
...
### `ind_selected` generated from `22_1_Spillovers_analysis.Rmd`

for (i in 1:length(ind_selected)) {
  ...
  fname <- paste0(dir.fig, 'Network/Network_top_', ind, '_lgl.png'); print(fname)
