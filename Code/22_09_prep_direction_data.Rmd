...
direction_ft  <- googlesheets4::read_sheet(link, sheet = 1) %>%
  as.data.frame() %>%
  dplyr::filter(
    !is.na(Used),
    !is.na(Filename),
    !grepl('\\?', Filename)) %>%
  dplyr::select(Filename, ShortName, Used) %>%
  arrange(Filename)
...
direction_clean <- direction %>%
  ...
  # dplyr::filter(Used == 1) %>%
  dplyr::select(FT_Indicators, ind, ShortName, Used, SDG_class, direction) %>%
  as.data.frame()
# str(direction_clean)
