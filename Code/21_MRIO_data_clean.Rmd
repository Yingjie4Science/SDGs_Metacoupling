...
inds <- substr(x = xls.ls.old, start = 4, stop = max(nchar(xls.ls.old))); inds
inds <- str_sub(string = inds, end = -10);
inds <- gsub(pattern = '_$', '', x = inds);
inds <- unique(inds); 
inds.old <- inds
print(paste0('The total number of Eora indicators: ', length(inds.old))) ## 54
...
for (i in inds) {
  ...
  if(length(xls.eora.ls) > n_year_check | length(xls.eora.ls) < (n_year_check-1) ) {
    ...
    ### save as xlsx
    fname <- paste0(dir.eora_cleaned, iname, '.xlsx'); print(fname); cat('\n\n')
...
```{r - arms trade, eval=FALSE, include=FALSE}
...
```{r - human_trafficking, eval=FALSE, include=FALSE}
...
```{r - student_flow, eval=FALSE, include=FALSE}
### get the indicator name
ind <- 'student_flow'
...
```{r - TradeMatrix_marine_capture, eval=FALSE, include=FALSE}
### get the indicator name
...
for (xls in xls.ls) {
  ...
  ### save plot
  fname <-paste0(dir.fig, 'footprint_heatmap/FT_HT_', iname, '_', yr, '_', postfix, '.png'); print(fname)
