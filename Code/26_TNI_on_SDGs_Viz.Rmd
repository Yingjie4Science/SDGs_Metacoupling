...
snr_labels <- c("real", "only distant", "only nearby", "none") %>% str_to_sentence()
...
dfs_score <- dfs %>%
  ...
  merge(x = .,
        y = grp %>% 
          ...
          dplyr::select(iso3, group) %>%
          dplyr::distinct(iso3, .keep_all = T),
        by = 'iso3', all.x = T) %>%
  dplyr::mutate(group = ifelse(group %in% c("High income", "Upper middle income"),
...
value_chg_max <- 20
value_chg_max <- 50 ## 6/15/2022, do we really need to limit the change range?

dfs_score_chg <- dfs_score %>%
  # dplyr::select(-group) %>%
  spread(key = scenario, value = score) %>%
  dplyr::mutate(value_chg = value_rel - value_not, 
                # value_chg = ifelse(value_chg >= value_chg_max,  value_chg_max, value_chg),
                # value_chg = ifelse(value_chg <=-value_chg_max, -value_chg_max, value_chg),
                value_chg = ifelse(iso3 =='GRL', NA, value_chg)) %>%    ## Greenland
  dplyr::mutate(value_chg_pct = value_chg/value_not) %>%  ## % of change
  # dplyr::select(-value_not, -value_rel) %>%
  dplyr::filter(year == 2015) %>%
  as.data.frame()

# hist(dfs_score_chg$value_chg)
...
p <- chg_err %>%
  ...
  ggplot(data = ., 
         aes(y = factor(goal, levels = rev(vars)), x = value_chg, fill = value_chg > 0)) +
  ...
  scale_fill_manual(values = c('#ffeda0', '#6baed6')) +
...
ggplot(data = chg_err_group, 
  ...
  theme_bar_my +
  theme(legend.position = c(0.86, 0.16),    ## 0.15, 0.06
        # legend.key.size = unit(0.2,"cm"), 
...
fname <- paste0(dir.fig, '01main/fig1c_SDG_score_chg_Bar_byGroup2grp_', today, '.jpg'); fname
ggsave(filename = fname, plot = last_plot(), width = 6, height = 8, units = 'in', dpi = 300)
```



```{r - map of 17 SDGs change - fig ex4}
...
fname <- paste0(dir.fig, '01main/SDG_score_chg_Map_byGoal_', today, '.jpg'); fname
...
col_tmap <- brewer.pal(n = 10, name = "Greens")

fname <- paste0(dir.fig, '01main/fig1d_SDG_score_count_Improved_', today, '.jpg'); fname
...
tm_shape(df_chg_sf) +
  tm_fill(col = 'percent_increase', 
          ...
          legend.reverse = T) +
  tm_borders(col = "grey", lwd = 0.1, lty = "solid", alpha = 0.99) +
  tm_credits(text = "d", fontface = "bold", size = 1.2, position = c(0, 0.9)) +
  tm_layout(
...
####  - Score by goal by scenario by group - fig 3 ex
...
p <-
  ...
  geom_errorbar(position=position_dodge(.9), width=.4, color = 'gray30', size = 0.3) +
  scale_fill_manual(
    name   = 'Scenarios',
    values = rev(snr_colors), 
    labels = rev(snr_labels), 
...
fname <- paste0(dir.fig, '01main/fig3ex_score_goal_snr4_grp_', today, '.jpg'); fname
...
### plot 
fname <- paste0(dir.fig, '01main/fig1ex_SDG_Index_Map_Baseline + Lockdown_', today, '.jpg'); fname
...
max(df_chg_sf$value_chg, na.rm = T) # 20
min(df_chg_sf$value_chg, na.rm = T) # -3.2
...
seq <- c(          -5, 0, 5, 10, 20); seq; length(seq)
col_tmap <- colors_changes[4:length(colors_changes)]
...
p <- tm_shape(df_chg_sf) +
  ...
  tm_layout(
    ...
    panel.show = F)
# dev.off()
tmap_save(tm = p, filename = fname, width=7, height=2.8, units="in", dpi = 300)
```



```{r - boxplot - change by 4 income group - fig ex2, eval=FALSE, include=FALSE}
dfs_index_chg %>%
...
fname <- paste0(dir.fig, '01main/Extend_fig_SDG_Index_chg_byGroup_4grp_', today, '.jpg'); fname
ggsave(filename = fname, plot = last_plot(), width = 6, height = 6, units = 'in', dpi = 300)
```


```{r - boxplot - change by 2 income group - fig ex2}
dfs_index_chg %>%
  # merge(., grp, by.x = 'iso3', by.y = 'iso_a3', all.x = T) %>%
  dplyr::mutate(
  ...
  ggplot(aes(x = group, y = value_chg, 
    ...
    geom_point(position=position_jitterdodge(jitter.width = 0.5),
               alpha = 0.6, show.legend = F) +
    scale_colour_manual(values = rev(color_grp)) +
    # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_hline(yintercept = 0, color="red", linetype="dashed") +
    xlab('') + ylab('Impact on SDG Index')
  
fname <- paste0(dir.fig, '01main/Extend_fig_SDG_Index_chg_byGroup_2grp_', today, '.jpg'); fname
ggsave(filename = fname, plot = last_plot(), width = 6, height = 6, units = 'in', dpi = 300)
```
...
map <- ggplot(data = df_sf) +
  ...
  theme_bw() +
  theme_map
fname <- paste0(dir.fig, 'map_each_SDG_indicator_score/', 'map_SDG_2scenarios_', ind_sdg_id, '_', ind_sdg_i, '_', today, '.jpg'); fname
