...
```{r ___ fig 1b - bar - change by Goal}
...
p1b <- chg_err %>%
  ...
  geom_bar(position = position_dodge(0.8), stat="identity", width = 0.8, show.legend = F, alpha = .7)+
  geom_errorbar(aes(
    xmin=value_chg-se,
    xmax=value_chg+se), 
    width=.3, color = 'gray20',
...
icons <- axis_canvas(plot = p1b, axis = 'y') + 
...
p_icon <- ggdraw(insert_yaxis_grob(plot = p1b, grob = icons, grid::unit(.1, "null"), position = "left"))
# p_icon

fig1b <- ggarrange(p_icon, labels = "b", font.label = list(size = 20), vjust = 1, hjust = 0)
fname <- paste0(dir.fig, '01main/fig1b_SDG_score_chg_Bar_Icon_', today, '.jpg'); fname
# ggsave(filename = fname, plot = fig1b, width = 6, height = 8, units = 'in', dpi = 600)
...
p1c <-
  ...
  geom_bar(position = position_dodge(0.8), stat="identity", width = 0.8, show.legend = T)+
  geom_errorbar(aes(
    ymin=value_chg-se, ymax=value_chg+se), 
    width=.3, color = 'gray20',
    position=position_dodge(.8)) +
  ylab("Transnational impact on SDG score") + xlab('SDGs') + labs(fill = "Group")+
  # scale_fill_brewer(palette = "Blues", direction = -1) +
  ...
  theme_bar_my +
  theme(
    legend.position = c(0.877, 0.15),    ## 0.15, 0.06
    # legend.key.size = unit(0.2,"cm"), 
...
ggarrange(p1c, labels = "c", font.label = list(size = 20), vjust = 1)
...
```{r ... fig1bc combined-1}
p1bc <- ggarrange(fig1b, p1c+xlab(''), labels = "", font.label = list(size = 20), vjust = 1, hjust = 0, align = 'v')
fname <- paste0(dir.fig, '01main/fig1Combined1_bc_SDG_score_chg_Bar_Icon_', today, '.jpg'); fname
ggsave(filename = fname, plot = p1bc, width = 6*2, height = 8, units = 'in', dpi = 600)
```
...
p <- chg_err_groups %>%
  ...
  theme_bar_my +
  theme(
    legend.position = c(0.877, 0.15),    ## 0.15, 0.06 -> 0.065
    # legend.key.size = unit(0.2,"cm"), 
    legend.box.background = element_rect(colour = "black"),
    legend.key.width  = unit(0.4,"cm"), 
    legend.key.height = unit(0.4,"cm"), 
...
###' need to define the break values and colors specifically -----------------------------
max(dfs_score_chg$value_chg, na.rm = T) ## 42.72
min(dfs_score_chg$value_chg, na.rm = T) ## -26.87468
...
col_tmap_score <- c(
  '#a63603','#e6550d','#fd8d3c',"#feb24c","#ffeda0", ## 6-class YlOrRd + 6-class Oranges
  '#eff3ff','#bdd7e7','#6baed6','#3182bd','#2171b5'  ## 6-class Blues
)

tm_shape(df_chg_sf) +
  tm_fill(vars, 
    palette = col_tmap_score,
    style = "fixed",
    breaks=c(-Inf, -20, -15, -10, -5, 0, 5, 10, 15, 20, Inf), 
    # midpoint = 0,
    labels = c("< -20", "[-20, -15)", "[-15, -10)", "[-10, -5]", "(-5, 0)", 
...
# dev.off()
tmap_save(tm = tmap_last(), filename = fname, width=7, height=6, units="in", dpi = 300, outer.margins = 0)
```
...
####  - Score by goal by scenario by group
...
saveRDS(dfs_index, file = f)


### global SDG index
dfs_index_glo <- dfs_index %>%
  group_by(year, scenario) %>%
  dplyr::summarise(index = mean(index, na.rm = TRUE)) %>%
  ungroup() %>%
  column_to_rownames(var = 'scenario') %>%
  dplyr::mutate(scenario = row.names(.),
                scenario = factor(x = scenario, levels = snr_levels)) %>%
  as.data.frame()



### add global sdg index change to Fig 3a
snr_chg_levels <- c('value_chg_rel', 'value_chg_dst', 'value_chg_ner')

dfs_index_glo_chg <- dfs_index_glo %>%
  spread(key = scenario, value = index) %>%
  dplyr::mutate(
    value_chg_rel = value_rel - value_not, 
    value_chg_dst = value_dst - value_not, 
    value_chg_ner = value_ner - value_not, 
    ) %>%
  dplyr::filter(year == 2015) %>%
  dplyr::select(year, starts_with('value_chg_')) %>%
  gather(key = 'scenario', value = 'index', value_chg_rel:ncol(.)) %>%
  dplyr::mutate(group = 'Global') %>%
  summarySE(measurevar= "index", groupvars = c("year", 'scenario', 'group'), na.rm = T) %>%
  dplyr::mutate(scenario = factor(x = scenario, levels = snr_chg_levels)) %>%
...
dfs_index_glo %>%
  ggplot(data = ., aes(x = scenario, y = index, fill = fct_rev(scenario))) +
  geom_col(width = .8, show.legend = F)+
  scale_fill_manual(
    name   = 'Scenarios',
    values = rev(snr_colors),
    labels = rev(snr_labels),
  ...
  scale_x_discrete(labels = snr_labels) +
  scale_y_continuous(limits = c(50, 60), breaks = seq(50, 60, 2), oob = rescale_none) +
  xlab('Scenarios') + ylab("Global SDG Index") +
  # theme_bar_my +
  theme(panel.grid = element_blank(),
...
#### - Index by scenario - bar
```{r ... fig3a}
dfs_index_snr <- dfs_index %>%
  summarySE(measurevar= "index", groupvars = c("year", 'scenario', 'group'), na.rm = T) %>%
  dplyr::arrange(year, scenario, group) %>%
  as.data.frame() 
...
max_score <- (max(dfs_index_sf$value_rel, na.rm = T)/5) %>% ceiling() %>% "*"(5); max_score # 75
min_score <- (min(dfs_index_sf$value_rel, na.rm = T)/5) %>% floor()   %>% "*"(5); min_score # 35
...
###' need to define the break values and colors specifically -----------------------------
break_seq_ini <- seq(-5, 20, 5)       

max(dfs_index_chg$value_chg, na.rm = T) ## 29.94641
min(dfs_index_chg$value_chg, na.rm = T) ## -5.548772
max_chg <- (max(dfs_index_chg$value_chg, na.rm = T)/5) %>% ceiling() %>% "*"(5); max_chg # 20   --> 30
min_chg <- (min(dfs_index_chg$value_chg, na.rm = T)/5) %>% floor()   %>% "*"(5); min_chg # -3.2 --> -10

c(
  "#feb24c","#ffeda0",
  '#eff3ff','#bdd7e7','#6baed6','#3182bd','#2171b5'
)
...
mean_change <- mean(dfs_index_chg_pct$`value_chg_pct`, na.rm = T) %>% round(digits = 1)
sd_change <- sd(dfs_index_chg_pct$`value_chg_pct`, na.rm = T) %>% round(digits = 1)

hist(dfs_index_chg_pct$value_chg)
boxplot(dfs_index_chg_pct$value_chg)
summary(dfs_index_chg_pct$value_chg)
dfs_index_chg_pct %>%
  ggplot(aes(x=value_chg, fill = value_chg<0)) +
  geom_histogram(binwidth=1, na.rm = T, alpha = 0.7, show.legend = F) +
  scale_fill_manual(values = c('#3182bd', '#feb24c')) +
  geom_vline(aes(xintercept=0), color="red", linetype="dashed", linewidth=1) +
  # ggplot2::annotate("0", x = 0, y = 25, label = "") +
  geom_vline(aes(xintercept=mean_change), color="black", alpha=0.6, linewidth=1.5) +
  scale_x_continuous(breaks = seq(-15, 30, 5)) +
  ggplot2::annotate("text", x = mean_change+3, y = 20, label = paste0("mean = ", mean_change, '\nsd = ', sd_change)) +
  xlab('Difference of national SDG Index between the real-world scenario and an autarky scenario') + 
  ylab('Number of copuntries') +
  theme_bw()
```
...
##' ! Since global SDG Index is rarely used in literature, we won't include such a score. 
##'   G_Index = (Index_c1 + Index_c2 + … + Index_c189)/189
##'   G_Index* = (Index*_c1 + Index*_c2 + … + Index*_c189)/189
# index_glo_chg <- (dfs_index_glo['value_rel', 'index'] - dfs_index_glo['value_not', 'index']) %>% round(., digits = 1)
# index_glo_chg_pct <- (index_glo_chg/dfs_index_glo['value_not', 'index'] * 100) %>% round(., digits = 1)
# 
# cat('Global SDG Index changed', index_glo_chg, '(or', index_glo_chg_pct, '% ). \n\n')
# # Global SDG Index changed 4.2 (or 7.7 % ). 


cat('SDG Index of countries would improve', 
    mean(dfs_index_chg_pct$value_chg, na.rm = T) %>% round(., digits = 1),      ## 9.2 -> 4.1
    'on average.\n')
cat('SDG Index of countries would improve', 
    paste0(mean_change,  ## 20.2% --> 9.1%
    '% (SD = ', sd_change, '%) on average.\n'))
...
n_improved <- sum(dfs_index_chg_count$increase, na.rm = T) 
pct_improved <- (n_improved/nrow(dfs_index_chg_count)*100) %>%   ## 96.8%
   round(., digits = 0)
cat(pct_improved, '% (n=', n_improved, ') of the world countries improved their SDG Index.\n')
...
cat('The remaining', 100-pct_improved, '% of the countries (n = ', 189-n_improved, 
    ') decreased their SDG Index.\n')
 
df_chg_sf_count_lowincome <- dfs_index_chg_count %>%
  dplyr::filter(increase < 1) %>%
  dplyr::filter(!is.na(group)) %>%
  dplyr::mutate(low_income = ifelse(
    stringr::str_detect(fixed(pattern = 'low', ignore_case = T), string = group), 1, 0)) %>%
  as.data.frame()
  
n_dec_low <- (sum(df_chg_sf_count_lowincome$low_income, na.rm = T)) 
(sum(df_chg_sf_count_lowincome$low_income, na.rm = T)/nrow(df_chg_sf_count_lowincome)*100) %>%
  round(., digits = 0) %>%
  cat('\t', ., '% (n=', n_dec_low, ') are low-income countries.')
## 76.5% are low-income
```
...
```{r - Index change - map}
dfs_index_chg_ <- dfs_index_chg %>%
  dplyr::select(-value_not, -value_rel)


### plot ---------------------------------------------------------------------------------
dfs_index_chg_sf  <- dfs_index_chg_ %>%
  merge(x = shp, y = ., by.x = "iso_a3", by.y = "iso3",  all.x = T) %>%
  dplyr::arrange(year, area, value_chg)


## - small regions that may be invisible on a map -----------------------
sf_small <- dfs_index_chg_sf %>%
...
sf_small_decrease <- dfs_index_chg_sf %>%
  dplyr::mutate(area = as.numeric(area)/10^6) %>%
...
sf_small_p <- st_read('./Data/country classification/country_small_annotation_points.shp') %>%
  ...
  merge(x = ., y = dfs_index_chg_, by.x = "ISO3", by.y = "iso3",  all.x = T) %>%
...
hist(dfs_index_chg_sf$value_chg)
...
col_tmap <- colors_changes[1:(length(break_seq))]
# col_tmap <- colors_changes
# col_tmap <- 'RdYlBu'

###' to make sure each categrory has the right color 
colors <- dfs_index_chg_sf %>% 
  dplyr::distinct(value_chg, chg_color) %>%
  arrange(value_chg) %>%
  dplyr::distinct(chg_color) %>%
  dplyr::filter(!is.na(chg_color))
col_tmap <- colors$chg_color
```
...
p <- tm_shape(dfs_index_chg_sf) +
  tm_fill(
    col = 'value_chg',
    stretch.palette = F, 
    # breaks = c(-Inf, min_chg, 0, 5, 10, 20, max_chg, Inf),
    
    ###' this option has to ensure the number of negative and positive breaks are the same
    ###'    so that the color can be symmetrical 
    # style  = "cont",
    # breaks = c(-30, -20, -15, -10, -5, 0, 5, 10, 15, 20, 30),
    # palette = 'RdYlBu',
    
    style  = "fixed",
    breaks = c(-Inf, break_seq_ini, Inf),
    palette = col_tmap, 
    legend.format = list(
      # scientific = TRUE, 
      # text.or.more = '<', text.less.than = '<',
      format="f", 
      text.separator = '–'),
    # labels = c("< -5", "[-5, 0)", "[0, 5)", "[5, 10)", "[10, 20]", paste0("> ", max_chg)),
    legend.reverse = T,
    textNA = 'NA', 
    colorNA = 'gray90',
    title  = 'Transnational\nimpact on\nSDG Index'
  ...
  tm_dots(
    ...
    style  = "fixed",
    breaks = c(-Inf, break_seq_ini, Inf),
    # breaks = c(-5, 0, 5, 10, 20, Inf),
    # labels = c("(-5, 0)", "[0, 5)", "[5, 10]", "(10, 20]", "> 20"),
...
p
# dev.off()
tmap_save(tm = p, filename = fname, width=7, height=2.8, units="in", dpi = 400)
```



```{r - change by 4 income group - boxplot}
```
...
```{r - change by 2 income group - boxplot}
```
...
dfs_index_chg_scr_summ <- dfs_index_chg_scr %>%
  dplyr::select(iso3, group, year, starts_with('value_chg_')) %>%
  gather(key = 'scenario', value = 'index', 4:ncol(.)) %>%
  summarySE(measurevar= "index", groupvars = c("year", 'scenario', 'group'), na.rm = T) %>%
  dplyr::mutate(scenario = factor(x = scenario, levels = snr_chg_levels)) %>%
  dplyr::arrange(year, scenario, group) %>%
  ### add global index data
  rbind(dfs_index_glo_chg) %>%
  as.data.frame() 

y_upper <- ceiling(max(dfs_index_chg_scr_summ$index, na.rm = T) + max(dfs_index_chg_scr_summ$se, na.rm = T) + 1)
y_lower <- floor(min(dfs_index_chg_scr_summ$index, na.rm = T) - max(dfs_index_chg_scr_summ$se, na.rm = T))


str(dfs_index_chg_scr_summ)

n_grp <- unique(dfs_index_chg_scr_summ$group) %>% length()
if (n_grp == 3) {
  pattern_fill <- c('High income' = "circle", 'Low income' = "none", 'Global' = 'crosshatch')
  legend.y <- 0.891
} else {
  pattern_fill <- c('High income' = "circle", 'Low income' = "none")
  legend.y <- 0.899
}

### plot ------------------
p <- dfs_index_chg_scr_summ %>%
  ggplot(data = ., 
         aes(x = scenario, y = index,
             ymin = index-se, ymax = index+se,
             fill = fct_rev(scenario), pattern = group
           )) +
  # geom_bar(position = position_dodge(0.8), stat="identity", width = 0.8, show.legend = T, alpha = .9)+
  ...
  scale_fill_manual(
    name   = 'Scenarios',
    values = rev(snr_colors[1:3]),
    labels = rev(snr_labels[1:3]),
    guide  = guide_legend(reverse = T)) +
  scale_x_discrete(labels = snr_labels[1:3]) +
  
  geom_errorbar(position=position_dodge(.9), width=.4, color = 'gray30') +
  scale_y_continuous(
    breaks = seq(y_lower, y_upper, 2),
    limits = c(y_lower, y_upper),
    oob = rescale_none
    ) +
  
  xlab('Scenarios') + ylab("SDG Index change") +
  # coord_flip() +
  theme_bar_my +
  theme(panel.grid = element_blank(),
        legend.position = c(0.875, legend.y), ## (0.894, 0.892)
        legend.box.background = element_rect(colour = "black"),
        legend.key.width = unit(0.4,"cm"),
        legend.key.height = unit(0.3,"cm"), 
        # legend.spacing.y = unit(0.0, 'cm'),
  ...
  guides(pattern = guide_legend(override.aes = list(fill = "white")),
         fill    = guide_legend(override.aes = list(pattern = "none"), reverse = T) ) 
p
p3a <- ggarrange(p, labels = "a", font.label = list(size = 20))
fname <- paste0(dir.fig, '01main/fig3a_index_chg_snr_', today, '_addGlobal.jpg'); fname
ggsave(filename = fname, plot = p3a, width = 6, height = 8, units = 'in', dpi = 300)
```
...
hist(df_score_chg_$value_chg)



df_score_chg_sf  <- df_score_chg_ %>%
  merge(x = shp, y = ., by.x = "iso_a3", by.y = "iso3",  all.x = T)
